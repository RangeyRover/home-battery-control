...
def parse_input_history(input_file):
    ...
    def interpolate(target_t):
        if target_t <= valid_data[0]['time']: return valid_data[0]['value']
        if target_t >= valid_data[-1]['time']: return valid_data[-1]['value']
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generate 24-hour average forecast directly from Home Assistant history.")
    parser.add_argument("--input", default="load_history.json", help="Path to input JSON file (default: load_history.json)")
    parser.add_argument("--out-json", default="average_24hr_forecast.json", help="Path to output JSON file (default: average_24hr_forecast.json)")
    parser.add_argument("--out-csv", default="average_24hr_forecast.csv", help="Path to output CSV file (default: average_24hr_forecast.csv)")
