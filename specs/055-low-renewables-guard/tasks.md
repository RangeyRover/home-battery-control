...
**Purpose**: Preserve renewables % in the rates pipeline — required by ALL guard stories

**⚠️ CRITICAL**: US1 (guard detection) cannot work without renewables data flowing through the pipeline
...
- [ ] T010 Run `pytest tests/test_rates.py -v -k "renewables"` — all 3 tests must pass

**Checkpoint**: Renewables % now flows through the full rates pipeline. Existing tests must still pass: `pytest tests/test_rates.py -v`

---

## Phase 3: User Story 1 — Automatic Low Renewables Detection (Priority: P1) 🎯 MVP

**Goal**: Detect low renewables from Amber Express forecast, activate/deactivate guard with hysteresis
...
**Checkpoint**: Guard logic works in isolation. No HA dependencies. `pytest tests/test_renewables_guard.py -v` passes.

---

## Phase 4: User Story 2 — Overnight SoC Deadline (Priority: P1)

**Goal**: When guard active, solver targets 100% SoC by 05:00 local time

**Independent Test**: Run LP solver with guard_deadline_steps containing the 05:00 step index and verify battery state reaches capacity at that step
...
- [ ] T025 [P] [US2] Write test `test_resolve_deadlines_past_deadline` in `tests/test_renewables_guard.py` — if current time is 08:00, only tomorrow's 05:00 is resolved (today's 05:00 has passed)
- [ ] T026 [P] [US2] Write test `test_guard_deadline_raises_battery_lower_bound` in `tests/test_fsm_lin.py` — pass guard_deadline_steps=[60], verify `b[60]` lower bound equals capacity
- [ ] T027 [P] [US2] Write test `test_guard_deadline_solver_charges_cheapest` in `tests/test_fsm_lin.py` — cheap overnight prices with deadline at step 60, verify plan shows CHARGE_GRID during cheap intervals
- [ ] T028 [P] [US2] Write test `test_guard_deadline_no_effect_when_none` in `tests/test_fsm_lin.py` — guard_deadline_steps=None → normal bounds unchanged
- [ ] T029 [P] [US2] Write test `test_guard_deadline_already_full` in `tests/test_fsm_lin.py` — battery at 100% SoC → solver doesn't over-charge (already satisfied)
- [ ] T030 [P] [US2] Write test `test_guard_deadline_coexists_with_no_import` in `tests/test_fsm_lin.py` — both no_import_steps and guard_deadline_steps active simultaneously, verify no conflict

### Implementation (make tests GREEN)

- [ ] T031 [US2] Add `resolve_deadline_steps()` method to `RenewablesGuard` in `custom_components/house_battery_control/renewables_guard.py` — iterate rates timeline, convert UTC→local time, match deadline hours, return step indices for today + tomorrow
- [ ] T032 [US2] Modify `propose_state_of_charge()` in `custom_components/house_battery_control/fsm/lin_fsm.py` to accept `guard_deadline_steps` parameter and raise `bounds[b_off + i]` lower bound to `capacity` for deadline steps (L192-200)
- [ ] T033 [US2] Modify `calculate_next_state()` in `custom_components/house_battery_control/fsm/lin_fsm.py` to extract `si.guard_deadline_steps` and pass to `propose_state_of_charge()`
- [ ] T034 [US2] Run `pytest tests/test_renewables_guard.py tests/test_fsm_lin.py -v -k "deadline or guard"` — all tests must pass

**Checkpoint**: Solver correctly targets 100% SoC at configured deadline steps. Full regression: `pytest tests/test_fsm_lin.py -v` passes.

---

## Phase 5: User Story 3 — Daytime SoC Deadline (Priority: P2)

**Goal**: When guard active, solver also targets 100% SoC by 15:00 local time

**Independent Test**: The daytime deadline uses the same mechanism as overnight — `resolve_deadline_steps()` already returns both deadlines. This phase validates the 15:00 behaviour specifically.

### Tests (write FIRST — must FAIL)

- [ ] T035 [P] [US3] Write test `test_resolve_deadlines_custom_times` in `tests/test_renewables_guard.py` — deadline at 04:00 and 14:00 → correct step indices
- [ ] T036 [P] [US3] Write test `test_guard_daytime_deadline_with_solar` in `tests/test_fsm_lin.py` — guard active, PV generating 2kW, deadline at step 180 (15:00), verify plan favours solar capture + charging toward deadline
- [ ] T037 [P] [US3] Write test `test_guard_daytime_deadline_no_solar` in `tests/test_fsm_lin.py` — guard active, PV=0, deadline at step 180, verify plan charges from grid at cheapest intervals

### Implementation (make tests GREEN)

- [ ] T038 [US3] Verify `resolve_deadline_steps()` correctly handles both overnight and daytime deadlines — the 15:00 deadline should already be resolved by T031. If any adjustment needed, update `custom_components/house_battery_control/renewables_guard.py`
- [ ] T039 [US3] Run `pytest tests/test_renewables_guard.py tests/test_fsm_lin.py -v -k "daytime or deadline"` — all tests must pass

**Checkpoint**: Both 05:00 and 15:00 deadlines work. Full regression passes.

---

## Phase 6: User Story 4 — Configuration Controls (Priority: P2)

**Goal**: Guard settings configurable via HA options flow

**Independent Test**: Change guard settings in options flow and verify the guard responds to new values
...
- [ ] T044 [US4] Run `pytest tests/test_config_flow.py -v -k "guard"` — all tests must pass

**Checkpoint**: Guard fully configurable from HA UI. `pytest tests/test_config_flow.py -v` passes.

---

## Phase 7: Coordinator Integration (Orchestration)

**Purpose**: Wire the guard into the coordinator update cycle
...
- [ ] T053 Run `pytest tests/test_coordinator.py -v -k "guard"` — all tests must pass

**Checkpoint**: Full end-to-end guard → solver → plan pipeline working. `pytest tests/test_coordinator.py -v` passes.

---

## Phase 8: User Story 5 — Dashboard Visibility (Priority: P3)
...
- [ ] T061 Run `pytest tests/test_renewables_guard.py tests/test_fsm_lin.py tests/test_rates.py tests/test_coordinator.py tests/test_config_flow.py -v` — all guard-related tests pass
- [ ] T062 [P] Update spec.md status from "Draft" to "Implemented" in `specs/055-low-renewables-guard/spec.md`
- [ ] T063 [P] Commit all changes and push branch `055-low-renewables-guard`
...
- **Phase 2 (Foundational)**: Depends on T001-T003 — renewables must flow through rates before guard can detect them
- **Phase 3 (US1 Detection)**: Depends on Phase 2 — needs renewables data
- **Phase 4 (US2 Overnight)**: Depends on Phase 3 — needs guard state to drive deadlines
- **Phase 5 (US3 Daytime)**: Depends on Phase 4 — extends deadline mechanism
- **Phase 6 (US4 Config)**: Can run in parallel with Phase 4/5 (different files: config_flow.py, strings.json)
- **Phase 7 (Coordinator)**: Depends on Phases 3+4+5+6 — wires everything together
- **Phase 8 (US5 Dashboard)**: Depends on Phase 7 — needs coordinator data keys
...
- T002, T003 are [P] — can run alongside T001
- T004-T006 are [P] — all test files, no conflicts
- T011-T020 are [P] — all in same new test file, but independent test functions
- T023-T030 are [P] — across two test files
- T040-T041 are [P] — different test file from implementation
- T045-T048 are [P] — coordinator tests
- Phase 6 (US4 Config) can run in parallel with Phases 4+5
...
1. Phase 1: Setup constants and dataclass fields
2. Phase 2: Renewables flows through rates pipeline
3. Phase 3: Guard detection logic works in isolation
4. Phase 4: Solver targets 100% at overnight deadline
5. **STOP and VALIDATE**: Guard detects + solver responds = core value delivered

### Full Delivery (Phases 5-9)

6. Phase 5: Daytime deadline (same mechanism, new time)
7. Phase 6: User can configure all settings
8. Phase 7: Wire into coordinator (live system)
9. Phase 8: Dashboard badge
10. Phase 9: Polish + full regression
...
- The solver change is just 3 lines in the bounds loop — minimal regression risk
