...
def process_ha_history(input_file, output_json, output_csv):
    ...
    print(
        f"Data spans {total_seconds / 86400:.2f} days. Generating {intervals} x 5-minute intervals."
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract and interpolate kWh usage from Home Assistant history."
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--out-json",
        default="7_day_kwh_usage.json",
        help="Path to output JSON file (default: 7_day_kwh_usage.json)",
    )
    parser.add_argument(
        "--out-csv",
        default="7_day_kwh_usage.csv",
        help="Path to output CSV file (default: 7_day_kwh_usage.csv)",
    )
