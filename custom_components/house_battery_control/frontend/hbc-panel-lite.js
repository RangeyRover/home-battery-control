...
import "./hbc-plan-table-lite.js";
import "./hbc-outlook-lite.js";
...
document.addEventListener("visibilitychange", () => {
  ...
  setTimeout(() => {
    if (!document.querySelector("hbc-panel-lite")) {
      location.reload();
    }
  }, 100);
});

class HBCPanelLite extends LitElement {
  ...
  connectedCallback() {
    ...
    this._fetchData();
    // 60s fallback timer for edge cases where entity state doesn't change
  ...
  async _fetchData() {
    try {
      ...
      // Fetch the LIGHTWEIGHT telemetry payload (< 5KB) instead of the 1MB status
      let resp = await this.hass.fetchWithAuth("/hbc/api/telemetry");
      if (resp.status === 401) {
        for (let i = 0; i < 5; i++) {
          await new Promise(r => setTimeout(r, 500));
          if (!this.hass) break;
          resp = await this.hass.fetchWithAuth("/hbc/api/telemetry");
  ...
  render() {
    ...
    return html`
      <div class="toolbar">
        ...
        <span class="toolbar-title">House Battery Control <a href="/hbc-debug" class="debug-link" title="Legacy Debug View">🪲</a></span>
        <div class="tabs">
          <button
      ...
      <div class="root">
        ${this._activeTab === "dashboard"
          ...
          ? html`
              <!-- Lazy loaded entirely by the child component -->
              <hbc-plan-table-lite .hass=${this.hass}></hbc-plan-table-lite>
            `
          : html`
              <!-- Explicit lazy loading via button in child component -->
              <hbc-outlook-lite .hass=${this.hass}></hbc-outlook-lite>
            `}
      </div>
    `;
  }
...
customElements.define("hbc-panel-lite", HBCPanelLite);
