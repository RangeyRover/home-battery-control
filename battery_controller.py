...
class BatteryContoller(object):
    ...
    def propose_state_of_charge(
        ...
        if self.optimizer is None or self.timestep_idx >= self.subsample:
            self.optimizer = self._get_optimizer(
                price_sell, price_buy, pv_forecast, load_forecast, current_balance, battery
            )
            self.timestep_idx = 0
    ...
    def _prepare_data_for_optimizer(
        self, price_sell, price_buy, pv_forecast, load_forecast, epochs_to_end, current_balance
    ):
        """
    ...
    def _get_optimizer(
        self, price_sell, price_buy, pv_forecast, load_forecast, current_balance, battery
    ):
        """
        ...
        period = self._prepare_data_for_optimizer(
            price_sell, price_buy, pv_forecast, load_forecast, epochs_to_end, current_balance
        )
        if epochs_to_end < self.FORECAST_LENGTH:
    ...
    def _get_current_balance_prediction(
        self, timestep_balance, previous_balance, site_id, timestamp
    ):
        """
...
def _get_selected_timestamp(ref_timestamp, timestamp_options):
    ...
    time_dist = [
        (ref_date - _get_datetime(_timestamp)).total_seconds() for _timestamp in timestamp_options
    ]
...
class PeriodOptimizer(object):
    ...
    def visualize_optimization(self):
        plot_period_summary_and_policy(
            self.period_summary.balance,
            self.period_summary.price_buy,
            self.policy,
    ...
    def visualize_fine_grain_policy(self):
        ...
        cost = compute_period_cost(
            timestep_balances, buy_prices, sell_prices, fine_grain_policy, self.battery
        )
...
def summarize_period(period, battery=None):
    ...
    if battery is not None:
        ...
        energy_balance = np.clip(
            energy_balance,
            battery.discharging_power_limit * 0.25,
            battery.charging_power_limit * 0.25,
        )
...
def _sincronize_battery_and_system(
    ...
    fine_grain_policy = np.clip(
        fine_grain_policy,
        np.min((initial_charge, final_charge)),
        np.max((initial_charge, final_charge)),
    ).tolist()
    return fine_grain_policy
...
def _sincronize_battery_and_system_for_timestep(
    initial_charge, final_charge, current_charge, balance, battery, timestep_balance
):
    """
    Provides a fine grain policy by sincronizing the battery and the system
    On a first step computes the ratio betwen the energy of the battery and the system.
    ...
    next_charge = np.clip(
        next_charge, np.min((initial_charge, final_charge)), np.max((initial_charge, final_charge))
    ).tolist()
    return next_charge
...
def apply_battery_power_limit(energy, hours, battery):
    return np.clip(
        energy, battery.discharging_power_limit * hours, battery.charging_power_limit * hours
    )
