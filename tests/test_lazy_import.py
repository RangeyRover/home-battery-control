...
def test_no_top_level_ortools_imports():
    """Assert ortools is never imported at the top-level scope of any integration file."""
            ...
            for node in tree.body:
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        assert not alias.name.startswith("ortools"), (
                            f"CRITICAL: Top-level import '{alias.name}' detected in {py_file}. "
                            f"PIP dependencies like OR-Tools MUST be lazy-loaded inside functions to avoid "
